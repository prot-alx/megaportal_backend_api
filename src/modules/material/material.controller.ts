...
export class MaterialController {
  ...
  @Post()
  @ApiOperation({ summary: 'Create a new material // Создать новый материал. При существовании такого же создать копию нельзя, необходимо работать с существующим, например, отредактировать через PUT.' })
  ...
  @Get()
  @ApiOperation({ summary: 'Get all materials // Вывести список всех материалов' })
  @ApiResponse({ status: 200, description: 'List of materials', type: [Material] })
  @ApiResponse({ status: 500, description: 'Internal server error' })
  ...
  async findOne(@Param('id') id: number): Promise<Material> {
    try {
      ...
      throw new DetailedInternalServerErrorException('Error retrieving material', error.message);
    }
  }

  @Put(':id')
  @ApiOperation({ summary: 'Update a material by ID // Изменить материал по ID' })
