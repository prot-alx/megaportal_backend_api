...
export class RequestsService {
  ...
  async create(createRequestDto: CreateRequestDto): Promise<Requests> {
    const { epId, requestDate, type, description, address, comment, status } =
      createRequestDto;

    // Fetch HR (or Dispatcher) from your system
    const hr = await this.employeeRepository.findOne({
      where: { role: EmployeeRole.Dispatcher },
    });
  ...
  async update(
    ...
    const hr = hrId
      ? await this.employeeRepository.findOne({ where: { id: hrId } })
      : request.hr;
